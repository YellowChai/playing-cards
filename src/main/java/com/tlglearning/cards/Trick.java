...
public class Trick {
  ...
  public static void main(String[] args) {
    ...
    // TODO Swap a random number of card between red and black piles.
    swapCards(rng, blackPile, redPile);
    // TODO Tally count of red card in red piles, black cards in black pile, and print result.
  ...
  private static void swapCards(Random rng, List<Card> blackPile, List<Card> redPile) {
    int swapCount = rng.nextInt(1 + Math.min(blackPile.size(), redPile.size())); // adding 1+ because nextInt will give you size()-1
  ...
  private static void tallyPiles(List<Card> blackPile, List<Card> redPile) {
    ...
    redPile.sort(
        Comparator
            .comparing(Card::getSuit, Comparator.comparing(Suit::getColor).reversed()) //red first
            .thenComparing(Comparator.naturalOrder())
    );
    System.out.printf(
